...
function search_value() {
    ...
    var package_name_path = $(".package_name_path")
    var package_upload_date = $(".package_upload_date");
    var package_version = $(".package_version");
    var package_icon = $(".package_icon")
    var package_size = $(".package_size")
    
    var package_name_links = $(".package_name_links")
    var package_version_links = $(".package_version_links")
    ...
    $.getJSON(url, function (json) {
        for (var i = 0; i < json.records.length; i++) {
            ...
            if (requested_package == json.records[i].package_name) {
                ...
                package_name_path.append(json.records[i].package_name)
                package_version.append(json.records[i].package_version)
                package_size.append(json.records[i].package_size + "MB")
                $(".download_btn").attr("href",json.records[i].package_link)
                $(".download_app").attr("href",json.records[i].package_link)
                
                package_upload_date.append(json.records[i].package_upload_date)
                package_name_links.append(json.records[i].package_name)
                package_version_links.append(json.records[i].package_version)
                package_mod_info.append(json.records[i].package_mod_info)
                check_point = 0

            }
            if (check_point == 1) {
                result = "Package cannot found!"
                result_class.className = "p-3 m-2 list-group-item list-group-item-warning rounded";
            }
